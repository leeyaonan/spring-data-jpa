...
public class ResumeDaoTest {
    ...
    public void testJpql(){
        List<Resume> list = resumeDao.findByJpql(1l, "蓝宝宝");
    ...
    @Test
    public void testSql(){
        List<Resume> list = resumeDao.findBySql("蓝%", "上海%");
        for (int i = 0; i < list.size(); i++) {
            Resume resume =  list.get(i);
            System.out.println(resume);
        }
    ...
    public void testMethodName(){
        List<Resume> list = resumeDao.findByNameLikeAndAddress("蓝%","上海");
        for (int i = 0; i < list.size(); i++) {
            Resume resume =  list.get(i);
            System.out.println(resume);
        }
    ...
    public void testSpecification(){
        ...
        Specification<Resume> specification = new Specification<Resume>() {
            ...
            public Predicate toPredicate(Root<Resume> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
                ...
                Predicate predicate = criteriaBuilder.equal(name, "蓝宝宝");
                return predicate;
            }
        };
    ...
    public void testSpecficationMultiCon(){
         ...
         *      需求：根据name（指定为"蓝宝宝"）并且，address 以"北"开头（模糊匹配），查询简历
         */
        ...
        Specification<Resume> specification = new Specification<Resume>() {
            ...
            public Predicate toPredicate(Root<Resume> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
                ...
                Predicate predicate1 = criteriaBuilder.equal(name, "蓝宝宝");
